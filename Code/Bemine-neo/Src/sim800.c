...
void	Gsm_InitValue(void)
{
	...
	HAL_Delay(1000);
  Gsm_UssdCancel();
	Gsm_MsgSetStoredOnSim(false);								//	select message stored to mudule
...
//#########################################################################################################
//#########################################################################################################
void GsmTask(void)//void const * argument)
{
	uint8_t GsmResult;
	Gsm_InitValue();
	//#######################	
...
//#########################################################################################################
//#########################################################################################################
void	Gsm_Init(void)//(osPriority	Priority)
{
	HAL_UART_Receive_IT(&_GSM_USART,&Gsm.usartBuff,1);
	Gsm_RxClear();
	Gsm_TxClear();
...
//void send(void)
//{	
//	sprintf((char*)Gsm.TxBuffer,"AT+COPN\r\n");
//	Gsm_SendString((char*)Gsm.TxBuffer);	
//}
...
{	
	HAL_Delay(100);
	CHANGE_UART_PINS(1);
	HAL_UART_Transmit(&_GSM_USART,Gsm.RxBuffer,20,100);
	CHANGE_UART_PINS(0);
}